...
def obter_pressao_grafico_sofascore(event_id):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if not points:
                return {'pico': 0, 'media': 0, 'recente': 0, 'aceleracao': 0, 'texto': ''}
            ...
            texto_fluxo = (
                f"🔥 *Pressão no Gráfico:* Pico `{pico_pressao:.0f}` | "
                f"Média `{media_abs:.1f}` | Recente `{recente_abs:.1f}`\n"
            )
...
def calcular_equilibrio(fin_h, fin_a):
    total = fin_h + fin_a
    if total <= 0:
        return 0
...
def calcular_score_05_ht(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h, fin_a, pressao, tem_prelive):
    score = 0
    motivos = []
    ...
    if tem_prelive:
        score += 2
        motivos.append("histórico pré-live propenso a gols")

    equilibrio = calcular_equilibrio(fin_h, fin_a)
    score += equilibrio

    base_ofensiva = (xg_tot >= 0.45 or chutes_gol >= 2 or fin_tot >= 7 or grandes_chances >= 1)
    pressao_valida = (pressao['pico'] >= 30 or pressao['recente'] >= 25 or chutes_gol >= 3 or grandes_chances >= 1)

    min_score = 10 if tem_prelive else 12
    aprovado = (score >= min_score and base_ofensiva and pressao_valida)
    return aprovado, score, motivos

def calcular_score_15_ht(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h, fin_a, pressao, tem_prelive):
    score = 0
    motivos = []
    ...
    if tem_prelive:
        score += 2
        motivos.append("tendência pré-live confirmada")

    equilibrio = calcular_equilibrio(fin_h, fin_a)
    score += equilibrio

    base_ofensiva = (xg_tot >= 0.70 or chutes_gol >= 3 or grandes_chances >= 1 or (fin_tot >= 8 and escanteios >= 3))
    pressao_valida = (pressao['pico'] >= 30 or pressao['recente'] >= 25 or chutes_gol >= 3)

    min_score = 12 if tem_prelive else 14
    aprovado = (score >= min_score and base_ofensiva and pressao_valida)
    return aprovado, score, motivos

def calcular_score_limite_ft(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h, fin_a, pressao, tem_prelive):
    score = 0
    motivos = []
    ...
    if tem_prelive:
        score += 2
        motivos.append("retrospecto pré-live propenso")

    equilibrio = calcular_equilibrio(fin_h, fin_a)
    score += equilibrio

    base_ofensiva = (xg_tot >= 1.20 or chutes_gol >= 4 or grandes_chances >= 2 or (fin_tot >= 12 and escanteios >= 5))
    pressao_valida = (pressao['pico'] >= 30 or pressao['recente'] >= 30 or chutes_gol >= 4)
...
def normalizar_score_100(score, mercado):
    maximos = {'05_HT': 23, '15_HT': 24, 'LIMITE_FT': 24}
    maximo = maximos.get(mercado, 24)
    return max(0, min(100, round((score / maximo) * 100)))

def calcular_confianca_elaborada(xg_tot, fin_tot, chutes_gol, grandes_chances, pressao, tem_prelive):
    score_vol = min(100, (fin_tot * 5) + (chutes_gol * 8))
    score_perigo = min(100, (xg_tot * 40) + (grandes_chances * 25))
    score_pressao = min(100, (pressao.get('recente', 0) * 1.5) + (pressao.get('aceleracao', 0) * 3))
    bonus_prelive = 10 if tem_prelive else 0

    fator_combinado = (score_vol * 0.35) + (score_perigo * 0.35) + (score_pressao * 0.20) + bonus_prelive
    confianca = max(7.5, min(9.9, round(fator_combinado / 10.0, 1)))
    return confianca
...
def montar_motivos_exibicao(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h, fin_a, intensidade, pressao_nivel, qualidade, tem_prelive):
    motivos = []
...
def montar_layout_alerta(mercado, liga, time_casa, time_fora, gols_c, gols_f, minutagem, score, xg_tot, xg_h, xg_a, fin_tot, fin_h, fin_a, chutes_gol, cg_h, cg_a, grandes_chances, gc_h, gc_a, escanteios, esc_h, esc_a, pressao, tem_prelive):
    score_100 = normalizar_score_100(score, mercado)
    intensidade = classificar_intensidade(pressao, fin_tot, chutes_gol, grandes_chances)
    pressao_nivel = classificar_pressao(pressao)
    qualidade = classificar_qualidade(xg_tot, chutes_gol, grandes_chances)
    
    confianca = calcular_confianca_elaborada(xg_tot, fin_tot, chutes_gol, grandes_chances, pressao, tem_prelive)
    motivos = montar_motivos_exibicao(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h, fin_a, intensidade, pressao_nivel, qualidade, tem_prelive)

    nomes_mercado = {
        '05_HT': ('🔥 SINAL +0,5 HT', 'OVER 0,5 HT'),
        '15_HT': ('🔥 SINAL +1,5 HT', 'OVER 1,5 HT'),
        'LIMITE_FT': (f'🎯 SINAL LIMITE FT (+{gols_c + gols_f + 0.5})', f'OVER LIMITE (+{gols_c + gols_f + 0.5})'),
    }
    titulo, faro = nomes_mercado.get(mercado, ('🔥 SINAL', 'OVER'))

    linhas_motivos = ''.join(f'• {motivo}\n' for motivo in motivos)
...
def checar_jogos_ao_vivo():
    ...
    try:
        ...
        for item in jogos:
            ...
            gc_tot, gc_h, gc_a = extrair_stat_sofascore(stats, 'Big chances')
            if gc_tot == 0:
                gc_tot, gc_h, gc_a = extrair_stat_sofascore(stats, 'Big chances created')
            ...
            prelive = obter_prelive_sofascore(event_id)
            tem_prelive = prelive is not None

            # 1. OVER 0.5 HT (0x0) -> 15' a 25'
            if total_gols == 0 and eh_1h:
                if event_id not in notificados_05_ht and 15 <= minuto_num <= 25:
                    aprovado, score, motivos = calcular_score_05_ht(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h_int, fin_a_int, pressao, tem_prelive)

                    if aprovado:
                        notificados_05_ht.add(event_id)
                        mensagem = montar_layout_alerta('05_HT', liga_formatada, time_casa, time_fora, gols_c, gols_f, minutagem, score, xg_tot, xg_h, xg_a, fin_tot, fin_h_int, fin_a_int, chutes_gol, cg_h_int, cg_a_int, grandes_chances, int(gc_h), int(gc_a), escanteios, esc_h_int, esc_a_int, pressao, tem_prelive)
                        msg_id = enviar_alerta(mensagem)
                        if msg_id:
                            alertas_pendentes[f"{event_id}_05_HT"] = {'event_id': event_id, 'message_id': msg_id, 'gols_alerta': total_gols, 'mercado': '05_HT', 'mensagem_original': mensagem}

            # 2. OVER 1.5 HT (1x0 / 0x1) -> 18' a 28'
            elif total_gols == 1 and eh_1h:
                if event_id not in notificados_15_ht and 18 <= minuto_num <= 28:
                    aprovado, score, motivos = calcular_score_15_ht(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h_int, fin_a_int, pressao, tem_prelive)

                    if aprovado:
                        notificados_15_ht.add(event_id)
                        mensagem = montar_layout_alerta('15_HT', liga_formatada, time_casa, time_fora, gols_c, gols_f, minutagem, score, xg_tot, xg_h, xg_a, fin_tot, fin_h_int, fin_a_int, chutes_gol, cg_h_int, cg_a_int, grandes_chances, int(gc_h), int(gc_a), escanteios, esc_h_int, esc_a_int, pressao, tem_prelive)
                        msg_id = enviar_alerta(mensagem)
                        if msg_id:
                            alertas_pendentes[f"{event_id}_15_HT"] = {'event_id': event_id, 'message_id': msg_id, 'gols_alerta': total_gols, 'mercado': '15_HT', 'mensagem_original': mensagem}
            ...
            # 3. OVER LIMITE FT -> 65' a 75'
                ...
                if event_id not in notificados_limite_ft and 65 <= minuto_num <= 75:
                    aprovado, score, motivos = calcular_score_limite_ft(xg_tot, fin_tot, chutes_gol, escanteios, grandes_chances, fin_h_int, fin_a_int, pressao, tem_prelive)

                    if aprovado:
                        notificados_limite_ft.add(event_id)
                        mensagem = montar_layout_alerta('LIMITE_FT', liga_formatada, time_casa, time_fora, gols_c, gols_f, minutagem, score, xg_tot, xg_h, xg_a, fin_tot, fin_h_int, fin_a_int, chutes_gol, cg_h_int, cg_a_int, grandes_chances, int(gc_h), int(gc_a), escanteios, esc_h_int, esc_a_int, pressao, tem_prelive)
                        msg_id = enviar_alerta(mensagem)
                        if msg_id:
                            alertas_pendentes[f"{event_id}_LIMITE_FT"] = {'event_id': event_id, 'message_id': msg_id, 'gols_alerta': total_gols, 'mercado': 'LIMITE_FT', 'mensagem_original': mensagem}
